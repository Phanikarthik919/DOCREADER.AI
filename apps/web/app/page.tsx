...
import {
  UploadCloud, ScanSearch, FilePenLine, Sparkles, Save, Bell, Search, ChevronDown, Trash2, Gem, TableIcon, PlusCircle, BrainCircuit, Bot, Download, User, UserCircle
} from 'lucide-react';
...
const DocReaderLogo = () => (
  <svg width="40" height="40" viewBox="0 0 40 40" fill="none"
    ...
    <path d="M12 28L12 8H28V28C28 29.1 27.1 30 26 30H14C12.9 30 12 29.1 12 28Z"
      fill="white" stroke="#7C3AED" strokeWidth="1" />
...
const createEmptyInvoice = (fileName: string): FullInvoice => ({
  ...
  invoice: {
    number: '', date: '', currency: '$', subtotal: 0,
    taxPercent: 0, total: 0, poNumber: '', poDate: ''
  },
  lineItems: [{
...
export default function Home() {
  ...
  const handleDownloadPdf = () => {
    ...
    autoTable(doc, {
      ...
      body: lineItems.map(li => [
        ...
        li.unitPrice.toFixed(2), li.total.toFixed(2)
      ]),
      startY: 70
    });
  ...
  const handleFormChange = (
    section: 'vendor' | 'invoice', field: string, value: string | number
  ) => {
    if (!invoiceData) return;
    setInvoiceData({
      ...invoiceData,
  ...
  const handleLineItemChange = (
    index: number, field: keyof Omit<LineItem, 'id' | 'total'>, value: string | number
  ) => {
    if (!invoiceData) return;
  ...
  const addLineItem = () => {
    ...
    setInvoiceData({
      ...
      lineItems: [
        ...
        { id: crypto.randomUUID(), description: '', unitPrice: 0, quantity: 1, total: 0 }
      ]
    });
  };
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <aside className="w-72 hidden md:flex flex-col p-6 border-r">
        <div className="flex items-center gap-3 mb-10">
          <DocReaderLogo />
          <h1 className="text-2xl font-bold">DOCREADER</h1>
        </div>
        ...
        <div className="mt-auto">
          <button className="w-full flex items-center gap-2 p-2 bg-purple-600 text-white rounded">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between p-6 border-b sticky top-0 z-10">
          <h2 className="text-3xl font-bold text-center w-full">Dashboard</h2>
          <div className="absolute right-6 flex items-center gap-2 relative">
            <button onClick={() => setShowBellDropdown(!showBellDropdown)}
              className="p-2 rounded hover:bg-gray-100">
              <Bell className="h-5 w-5" />
            </button>
            {showBellDropdown && (
              <div className="absolute right-0 mt-12 w-64 p-2 rounded-xl border shadow-lg bg-white">
                Notifications...
              </div>
            )}
            ...
            {showProfileDropdown && (
              <div className="absolute right-0 mt-12 w-48 p-2 rounded-xl border shadow-lg bg-white">
                Profile...
              </div>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col p-6 gap-6 overflow-y-auto">
          <PanelGroup direction="horizontal" className="flex-1 min-h-[60vh]">
              ...
              <div className="w-full h-full flex flex-col rounded-2xl border overflow-hidden">
                <div className="p-4 border-b flex justify-between items-center">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <ScanSearch className="w-5 h-5" /> Document Viewer
                  </h3>
                ...
                <div className="flex-1 p-4">
                  {pdfUrl ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center rounded-2xl border-2 border-dashed">
                      <UploadCloud className="w-16 h-16 mb-4" />
                      <p className="font-semibold text-lg">Upload a document</p>
                    </div>
                  )}
            ...
            <PanelResizeHandle className="w-2 flex items-center justify-center group">
              <div className="w-1 h-10 rounded-full group-hover:opacity-80 bg-gray-300" />
            </PanelResizeHandle>
            ...
            <Panel defaultSize={35} minSize={25}>
              <div className="w-full h-full flex flex-col rounded-2xl border overflow-hidden">
                <div className="p-4 border-b">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <FilePenLine className="w-5 h-5" /> Data Extraction
                  </h3>
                </div>
                  ...
                  <div className="flex gap-4">
                    <label>
                      <input type="radio"
                        ...
                        onChange={() => setAiProvider('gemini')} /> Gemini
                    </label>
                    <label>
                      <input type="radio"
                        checked={aiProvider === 'groq'}
                        onChange={() => setAiProvider('groq')} /> Groq
                    </label>
                  </div>

                  {!invoiceData ? (
                    <div className="text-center pt-10">
                      Upload and extract data to see the form.
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input className="border p-1 rounded"
                          placeholder="Name" value={invoiceData.vendor.name}
                          onChange={e => handleFormChange('vendor', 'name', e.target.value)} />
                        <input className="border p-1 rounded"
                          placeholder="Tax ID" value={invoiceData.vendor.taxId}
                          onChange={e => handleFormChange('vendor', 'taxId', e.target.value)} />
                        <input className="border p-1 rounded col-span-2"
                          placeholder="Address" value={invoiceData.vendor.address}
                          onChange={e => handleFormChange('vendor', 'address', e.target.value)} />
                      </div>

                      <h4 className="font-semibold pt-4">Invoice</h4>
                      <div className="grid grid-cols-2 gap-2">
                        <input className="border p-1 rounded"
                          placeholder="Number" value={invoiceData.invoice.number}
                          onChange={e => handleFormChange('invoice', 'number', e.target.value)} />
                        <input className="border p-1 rounded"
                          placeholder="Date" value={invoiceData.invoice.date}
                          onChange={e => handleFormChange('invoice', 'date', e.target.value)} />
                        <input className="border p-1 rounded"
                          placeholder="Total" type="number"
                          value={invoiceData.invoice.total}
                          onChange={e => handleFormChange('invoice', 'total', parseFloat(e.target.value))} />
                        <input className="border p-1 rounded"
                          placeholder="Currency" value={invoiceData.invoice.currency}
                          onChange={e => handleFormChange('invoice', 'currency', e.target.value)} />
                      </div>

                      <h4 className="font-semibold pt-4 flex justify-between items-center">
                        Line Items
                          ...
                          {invoiceData.lineItems.map((item, index) => (
                            <tr key={item.id}>
                              <td className="border p-1">
                                <input
                                  className="w-full h-full p-1 box-border text-center border-none outline-none truncate"
                              ...
                              <td className="border p-1">
                                <input
                                  type="number"
                                  className="w-full h-full p-1 box-border text-center border-none outline-none"
                                  value={item.quantity}
                ...
                <div className="p-6 border-t grid gap-4">
                  <button onClick={handleExtract}
                    className="w-full px-4 py-2 bg-blue-600 text-white rounded flex justify-center items-center gap-2">
                    <Sparkles className="w-4 h-4" /> {isExtracting ? "Extracting..." : "Extract with AI"}
                  </button>
                  <button onClick={handleSaveInvoice}
                    className="w-full px-4 py-2 bg-green-600 text-white rounded flex justify-center items-center gap-2">
                    <Save className="w-4 h-4" /> Save Invoice
                  </button>
                  <button onClick={handleDownloadPdf}
                    className="w-full px-4 py-2 bg-purple-600 text-white rounded flex justify-center items-center gap-2">
          ...
          <section className="w-full border rounded-xl p-6">
            <h3 className="text-xl font-bold text-center mb-4"># Saved Invoices</h3>
            {savedInvoices.length === 0 ? (
              <p className="text-center text-gray-500">No invoices saved yet.</p>
            ) : (
              <table className="w-full border-collapse border border-gray-300 text-center">
                <thead className="bg-gray-200">
                  <tr>
                ...
                <tbody>
                  {savedInvoices.map(inv => (
                    <tr key={inv._id}>
                      ...
                      <td className="border p-2">{inv.invoice?.date || 'N/A'}</td>
